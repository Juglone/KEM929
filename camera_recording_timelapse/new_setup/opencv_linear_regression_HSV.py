...
def process_video(video_path):
    ...
    while cap.isOpened():
        ...
        hsv_values.append(
            cv2.cvtColor(
                square_frame, cv2.COLOR_RGB2HSV
            )[:, :, 1]
            .astype("float32").mean() / 255
        )
...
def plot_values(video_path, hsv_values):
    # Convert list to numpy array
    hsv_values = np.array(hsv_values)
    hours = np.arange(len(hsv_values))*200/(30*3600)

    # Perform linear regression using scipy
    slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(hours, hsv_values)
    ...
    # Plot HSV values over time with linear regression line
    plt.plot(hours, hsv_values, label='Data')
    plt.plot(hours, slope * hours + intercept, 'r', label=f'Linear Fit (slope={slope:.4f})')
    plt.xlabel('Hours')
    plt.ylabel('Average HSV Value')
    title_text = (
        f"HSV Value Over Time for Gel in a Medium with {pH_value} and {IS_value}, \n"
        f"{condition}, {date}"
    )
    ...
    # Save plot to folder
    output_folder = "hsv_graphs"
    output_filename = "hsvplot_" + video_path.split('.')[0] + ".png"
