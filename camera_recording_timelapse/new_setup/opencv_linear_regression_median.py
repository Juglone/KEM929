...
def process_video(video_path):
    ...
    while cap.isOpened():
        ...
        grayscale_values.append(np.median(cv2.resize(square_frame.astype("float32"), (10, 10), interpolation =
                                           cv2.INTER_AREA)) / 255)
...
def plot_values(video_path, grayscale_values):
    ...
    plt.xlabel('Hours')
    plt.ylabel('Median Grayscale Value')
    title_text = (
        f"Grayscale Median Pixelvalue Over Time for Gel in a Medium with {pH_value} and {IS_value}, \n"
        f"{condition}, {date}"
    )
    ...
    # Save plot to folder
    output_folder = "grayscale_median_graphs"
    output_filename = "grayscale_median_plot_" + video_path.split('.')[0] + ".png"
